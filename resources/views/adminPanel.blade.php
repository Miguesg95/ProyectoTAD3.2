...
<main class="container panel py-3 my-4 text-black border-top border-bottom">
    ...
    <section id="listar">
        <div class="container" id="hanging-icons">
            <div class="row py-5 justify-content-center text-center">
                ...
                <div class="col">
                    <div>
                        <button class="navbar-toggler rounded border text-white fs-5 p-2 bgButton" type="button"
                            data-bs-toggle="collapse" data-bs-target="#navbarEstadisticas">
                            Estadisticas
                        </button>
                    </div>
                </div>
                <div class="col">
                    <div>
                        <button class="navbar-toggler rounded border text-white fs-5 p-2 bgButton" type="button"
                            data-bs-toggle="collapse" data-bs-target="#navbarCategorias">
                            Categorias
                        </button>
                    </div>
                </div>
        ...
        <div class="collapse mb-4 mx-4" id="navbarCalzados">
                            ...
                            <tr>
                                <form action="{{ route('calzadoAdmin.actualizar', $calzado->id) }}" method="POST">
                                    ...
                                    <td scope="row" class="align-middle border-dark text-start border col-md-3">
                                        <input type="text" class="form-control" id="validationDefault01" name="nombre"
                                            value="{{ $calzado->nombre }}" required>
                                    </td>
                                    <td class="align-middle border-dark border col-md-2">
                                        <input type="text" class="form-control" id="validationDefault01" name="marca"
                                            value="{{ $calzado->marca }}" required>
                                    </td>
                                    <td class="align-middle border-dark border col-md-4">
                                        <input type="text" class="form-control" id="validationDefault01" name="descripcion"
                                            value="{{ $calzado->descripcion }}" required>
                                    </td>
                                    <td class="align-middle border-dark border col-md-1">
                                        <input type="number" step="0.01" class="form-control" id="validationDefault02"
                                            name="precio" value="{{ $calzado->precio }}" required>
                                    </td>
                                    <td class="align-middle border-dark border col-md-1">
                                        <input type="number" step="0.01" class="form-control" id="validationDefault02"
                                            name="stock" value="{{ $calzado->stock }}" required>
                                    </td>
                                    <td class="align-middle border-dark border"><button
                                            class="border text-white bgButton rounded" type="submit">Actualizar</button>
                                    </td>
                                </form>
                                <td class="align-middle border-dark border">
                                    <form action="{{ route('calzadoAdmin.eliminar', $calzado->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <button class="border text-white bgButton rounded" type="submit">Eliminar</button>
                                </td>
                                </form>
                                </td>
                            </tr>
            ...
            <form class="row" action="{{ route('calzadoAdmin.crear') }}" method="POST">
                ...
                <div class="col-md-1">
                    ...
                    <input type="number" step="0.01" class="form-control" id="validationDefault02" name="precio"
                        required>
                </div>
                <div class="col-md-1">
                        ...
                        @foreach ($users as $user)
                            <tr>
                                <form action="{{ route('userAdmin.actualizar', $user->id) }}" method="POST">
                                    ...
                                    @csrf
                                    <td scope="row" class="align-middle border-dark text-start border col-md-3">
                                        <input type="text" class="form-control" id="validationDefault01" name="username"
                            ...
                            <tr>
                                ...
                                <td class="border-dark border col-md-4">
                                    @foreach ($carrito->lineaDeCarritos as $linea)
                                        <p class="text-start"><a href="{{ route('lineaDeCarritoAdmin.eliminar', $linea->id) }}"
        ...
        <div class="collapse mb-4 mx-4 show" id="navbarVentas">
            ...
            <form class="row" action="{{ route('ventaAdmin.crear') }}" method="POST">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="validationDefault04" name="carrito_id" required>
                        <option selected disabled value="">....</option>
                        @foreach ($carritos as $carrito)
                            <option value="{{ $carrito->id }}">{{ $carrito->user->username }} - {{ $carrito->user->email }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    ...
                    <input type="text" class="form-control" id="validationDefault01" name="estado" value="Confirmado"
                        required>
                </div>
                <div class="col-12 my-3 ">
                    <button class="border rounded text-white bgButton py-1 px-2" type="submit">Enviar</button>
        ...
        <div class="collapse mb-4 mx-4" id="navbarEstadisticas">
            <h3>Productos guardados más en favoritos</h3>
            <div class="table-responsive">
                <table class="table text-center border table-striped">
                    <thead class="table-primary">
                        <tr>
        ...
        <div class="collapse mb-4 mx-4" id="navbarCategorias">
                        ...
                        @foreach ($categorias as $categoria)
                            <tr>
                                <form action="{{ route('categoria.actualizar', $categoria->id) }}" method="POST">
                                    @method('PUT')
                                    @csrf
                                    <td scope="row" class="align-middle border-dark text-start border col-md-10">
                                        <input type="text" class="form-control" name="nombre"
                                            value="{{ $categoria->nombre }}" required>
                                    </td>
                                    <td class="align-middle border-dark border">
                                        <button class="border text-white bgButton rounded" type="submit">Actualizar</button>
                                    </td>
                                </form>
                                <td class="align-middle border-dark border">
            ...
            @endif
            <form class="row" action="{{ route('categoria.crear') }}" method="POST">
                @csrf
                <div class="col-md-10">
            ...
            <br>
            <h3>Asignar Categorías a Calzados</h3>
            <form class="row" action="{{ route('calzado.asignarCategorias') }}" method="POST">
                @csrf
                <div class="col-md-6">
                    <label for="calzado" class="form-label">Calzado</label>
                    <select class="form-control" id="calzado" name="calzado_id" required>
                        @foreach ($calzados as $calzado)
                            <option value="{{ $calzado->id }}">{{ $calzado->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-12 my-3">
                    <button class="border rounded text-white bgButton py-1 px-2" type="submit">Asignar
                        Categorías</button>
                </div>
            </form>
        </div>
